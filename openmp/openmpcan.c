...
double *** init_array(int procdim, int nlocal, int random) {
	...
	for (i = 0; i < procdim; i++) {
		...
		for(j = 0; j < procdim; j++) {
			...
			if(random)
				for(k = 0; k < nlocal * nlocal; k++) { arr[i][j][k] = fRand(); }
			else 
				for(k = 0; k < nlocal * nlocal; k++) { arr[i][j][k] = 0.00; }
		}
	}
	return arr;
}
